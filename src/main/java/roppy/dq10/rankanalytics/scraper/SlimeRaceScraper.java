...
public class SlimeRaceScraper implements Scraper {
    ...
    public List<List<RankItem>> scrape(int round, int subraceNumber) throws IOException,ScrapingException {
        ...
        for(int page = 0; page < PAGE_NUMBER;page++) {
            ...
            String url = String.format("https://hiroba.dqx.jp/sc/worldRanking/slimeracingRanking/%d/page/%d",round,page);
            ...
            for (Element ranker : rankers) {
                ...
                r.setRank(rank);
                r.setName(ranker.select(".col3 .pointget-col32-text").get(0).html().trim());
                r.setExtraData(ranker.select(".col4 .pointget-col32-text").get(0).html().trim());
                int point;
                try{
                    point = Integer.parseInt(ranker.select(".col2").get(0).html().replace("&nbsp;P", "").replace(",", "").trim());
                }catch (NumberFormatException nfe){
                    throw new ScrapingException(nfe);
                }
                r.setPoint(point);
