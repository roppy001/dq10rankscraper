...
public class BattlePencilScraper implements Scraper{
    ...
    public List<List<RankItem>> scrape(int round, int subraceNumber) throws IOException,ScrapingException {
        ...
        for(int page = 0; page < PAGE_NUMBER;page++) {
            ...
            String url = String.format("https://hiroba.dqx.jp/sc/worldRanking/battlePencil/%d/page/%d",round,page);
            ...
            for (Element ranker : rankers) {
                ...
                int point;
                try{
                    point = Integer.parseInt(ranker.select(".col3 .pointget-col34").get(0).html().replace("P", "").trim());
                }catch (NumberFormatException nfe){
                    throw new ScrapingException(nfe);
                }
                r.setPoint(point);
