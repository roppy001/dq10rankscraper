...
public class FishingContestScraper implements Scraper{
    private static final int PAGE_NUMBER = 10;
    private static final String [] URL_SUBRACE_MAPPER = {"","sub/"};

    @Override
    public List<List<RankItem>> scrape(int round, int subraceNumber) throws IOException, ScrapingException {
        if(subraceNumber != 1 &&  subraceNumber != 2){
        ...
        for(int subraceIndex = 0;subraceIndex < subraceNumber;subraceIndex++) {
            ...
            for(int page = 0; page < PAGE_NUMBER;page++) {
                ...
                String url = String.format("https://hiroba.dqx.jp/sc/worldRanking/fishingContest/%s%d/page/%d",URL_SUBRACE_MAPPER[subraceIndex],round,page);
                ...
                for (Element ranker : rankers) {
                    ...
                    int point;
                    try{
                        point = Integer.parseInt(ranker.select(".col3 .pointget-col34").get(0).html().replace("cm", "").replace(".", "").trim());
                    }catch (NumberFormatException nfe){
                        throw new ScrapingException(nfe);
                    }
                    r.setPoint(point);
